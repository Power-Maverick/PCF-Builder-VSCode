    ...
    public GetCompletionProvider(): vscode.Disposable {
        let manifestCompletionProvider = vscode.languages.registerCompletionItemProvider(this.docSelector, {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position, token: vscode.CancellationToken, context: vscode.CompletionContext) {
                ...
                snipProperty.insertText = new vscode.SnippetString('<property name="${1}" display-name-key="${2}" description-key="${3}" of-type="${4}" usage="${5|bound,input|}" required="${6|true,false|}" />');
                snipProperty.documentation = new vscode.MarkdownString("Property node identifies a specific, configurable piece of data that the control expects from CDS.");
                ...
                snipTypeGroup.insertText = new vscode.SnippetString('<type-group name="${1}">\n\t<type>${2}</type>\n</type-group>');
                snipTypeGroup.documentation = new vscode.MarkdownString("Group multiple types on a property using of-type-group.");
                ...
                snipResources.insertText = new vscode.SnippetString('<resources>\n\t<code path="index.ts" order="1" />\n</resources>');
                snipResources.documentation = new vscode.MarkdownString("The resources node in the component manifest refers to the resource file that components require to implement it's visualization.");
                ...
                snipFeatures.insertText = new vscode.SnippetString('<feature-usage>\n\t<uses-feature name="${1}" required="${2|true,false|}" />\n</feature-usage>');
                snipFeatures.documentation = new vscode.MarkdownString("Allow developers to declare which features their component wants to use.");
                ...
                snipFeatureUses.insertText = new vscode.SnippetString('<uses-feature name="${1}" required="${2|true,false|}" />');
                snipFeatureUses.documentation = new vscode.MarkdownString("Indicates which feature their components want to use.");
